...
def page (a):
	...
	# keep looping
	if a == 1:
		espeak.synth("show corner 1")
		time.sleep(1)
	if a == 2:
		espeak.synth("show corner 2")
		time.sleep(1)	
	if a == 3:
		espeak.synth("show corner 3")
		time.sleep(1)
	if a == 4:
		espeak.synth("show corner 4")
		time.sleep(1)
	while True:
		...
		if (key == ord('q')) or  (len(pts)>24 and np.abs(dX) < 5 and np.abs(dY) < 5):
			...
			break
		if t == 0:
			espeak.synth("the corner not visible")
			time.sleep(1.5)
			t = 250	
...
def crop (M2,M5):
	...
	while True:
		...
		if l ==1:
			while True:
				ret,frame2 = camera.read()
				frame2 = cv2.warpPerspective(frame2,M5,(600,900))
				cv2.imshow('frame', frame2)
